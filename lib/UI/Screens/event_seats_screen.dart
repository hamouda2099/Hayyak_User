...
class EventSeatsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    print(selectedDate);
    return
      FutureBuilder<EventSeatsModel>(
        future: ApiManger.getEventSeats(
            date: selectedDate.substring(0, 10), eventId: eventId),
        builder:
            (BuildContext context, AsyncSnapshot<EventSeatsModel> snapShot) {
                              ...
                              Column(
                                children: List.generate(snapShot.data!.data.event.kinds.length, (index){
                                  return SeatCategoryComponent(
                                    seatCategory: snapShot.data!.data.event.kinds[index],
                                  );
                                }),
                              )
                                  :
                              Column(
                                children: List.generate(TestDate().seatsCategory.length, (index){
